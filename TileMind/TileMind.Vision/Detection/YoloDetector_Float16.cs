...
namespace TileMind.Vision.Detection
{
    ...
    {
        extension(YoloDetector yoloDetector)
        {
            internal float Float16BitsToSingle(ushort hbits)
            {
            ...
            internal float Float16ToSingle(Microsoft.ML.OnnxRuntime.Float16 v)
            {
            ...
            internal List<NamedOnnxValue> PreprocessImageFloat(Mat image, string inputName, int inputHeight, int inputWidth)
            {

                Mat letterboxedImage;
                if (inputHeight != -1 || inputWidth != -1)
                {
                    letterboxedImage = new Mat(inputHeight, inputWidth, MatType.CV_8UC3, Scalar.Gray);
            ...
            internal List<DetectionResult> PostprocessOutputFloat16(
